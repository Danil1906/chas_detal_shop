...
class Category(MPTTModel):
    ...
    photo = models.ImageField(upload_to='photos_category/%Y/%m/%d/', blank=True, default='default.webp')
    ...
    def save(self, *args, **kwargs):
        ...
        if name != 'default':
            webp_name = self.photo.path.split('.')[0] + '.webp'
            if image.height > 400 or image.width > 400:
                resize = (400, 400)
                image.thumbnail(resize)

                # image.save(self.photo.path, quality=20, optimize=True)
                image.save(webp_name, format='WebP', lossless=True)
                self.photo = webp_name
                super().save()
            else:
                image.save(webp_name, format='WebP', lossless=True)
                self.photo = webp_name
...
class Product(models.Model):
    ...
    photo = models.ImageField(upload_to='photos/%Y/%m/%d/', blank=True, default='default_prod_black.webp')
    ...
    def get_absolute_url(self):
        return reverse('product', kwargs={'slug': self.slug, 'cslug': self.category.slug})
    ...
    def save(self, *args, **kwargs):
        ...
        if self.quantity > 0 and len(self.notify_list) > 0:
            ...
            send_mail(
                ...
                f'''Товар http://{self.title} появился в наличии в количестве {self.quantity} шт. и доступен к покупке. ChasDetal.
                ''',
                'chasdetal@gmail.com',
                list_email
            )
...
class SliderBanner(models.Model):
    ...
    photo = models.ImageField(upload_to='backgrounds/%Y/%m/%d/', blank=True, help_text='''Для большого слайдера размер 966*400 (цвет фона страницы #f3efef) 
