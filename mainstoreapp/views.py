...
class DetailCategoryView(ListView):
    ...
    def get_queryset(self):
        ...
        if category:
            ...
            base = Product.objects.filter(category=Category.objects.get(slug=self.kwargs['slug'])).select_related(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail = send_mail(form.cleaned_data['subject'], content, 'chasdetal@gmail.com',
                             ['danilkozlov36@yandex.ru'], fail_silently=False)
            if mail:
